    ...
    def _parse_odata_schema(data) -> Tuple[dict, dict]:
        ...
        for entity_type in tqdm.tqdm(entity_types, "Parsing OData schema"):
            ...
            for prop in entity_type.findall("edm:Property", namespaces):
                ...
                try:
                    if prop_name is not None:
                        ...
                        entities[entity_name]["columns"][prop_name]["type"] = prop.get("Type").split(".")[-1]
                        col_des = entity_name
                        if len(prop.findall("edm:Annotation", namespaces)) > 0:
                            if len(prop.findall("edm:Annotation", namespaces)[0].get("String")) > 0:
                                col_des = prop.findall("edm:Annotation", namespaces)[0].get("String")
            ...
            if len(description) > 0:
                ...
                try:
                    entities[entity_name]["description"] = entity_name + " with Primery key: " + entity_type.find("edm:Key/edm:PropertyRef", namespaces).attrib['Name']
                except:
            ...
            for rel in entity_type.findall("edm:NavigationProperty", namespaces):
                ...
                # src_col, tgt_col = guess_relationship_columns(source_entity, target_entity, entities[source_entity], entities[target_entity])
                    relationships[rel_name].append({
                                        ...
                                        "note": "inferred" if source_col and target_col else "implicit/subform"
                                    })
