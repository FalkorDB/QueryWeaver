...
exports.QueryWeaverClient = void 0;
...
class QueryWeaverClient {
    ...
    async raiseForStatus(response) {
        if (!response.ok) {
            ...
            throw new types_1.APIError(errorMessage, response.status, responseData);
        }
    }
    /**
     ...
     */
    async query(graphId, chatData, autoConfirm = false) {
        const response = await fetch(this.url(`/graphs/${graphId}`), {
            method: 'POST',
            headers: this.defaultHeaders,
     ...
     */
    async confirm(graphId, confirmData) {
     ...
     */
    async consumeStreamingResponse(response) {
        const events = [];
        if (!response.body) {
            return {};
        }
...
exports.QueryWeaverClient = QueryWeaverClient;
