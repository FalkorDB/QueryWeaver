...
"""
        ...
        prompt = f"""
            ...
            Data-modification requests are IN SCOPE and MUST be translated into the appropriate INSERT/UPDATE/DELETE statement. Do NOT refuse them, and do NOT set "is_sql_translatable" to false, merely because the request is not a SELECT — being a write operation is never, by itself, a reason to treat a request as untranslatable. A downstream confirmation step reviews destructive operations before they run; your job is only to produce the correct single SQL statement.
            ...
            S2. Single statement: Output exactly ONE valid SQL statement that answers the user question using the schema — a SELECT for data retrieval, or an INSERT/UPDATE/DELETE when the user explicitly requests to add, modify, or remove data (not a fixed/constant response unless the question explicitly asks for a constant).
