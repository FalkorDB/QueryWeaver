...
def llm_answer_validator(
    question: str, answer: str, expected_answer: str = None
) -> str:
    prompt = """
...
def llm_table_validator(
    question: str, answer: str, tables: List[str]
) -> Tuple[float, str]:
    prompt = """
    ...
    response = completion(
        ...
        messages=[
            {"role": "system", "content": "You are a Validator assistant."},
            {
                "role": "user",
                "content": prompt.format(
                    question=question, tables=tables, generated_answer=answer
                ),
            },
        ],
        response_format={"type": "json_object"},
