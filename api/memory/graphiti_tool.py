...
class MemoryTool:
    ...
    async def add_new_memory(self, conversation: Dict[str, Any]) -> bool:
        # Use LLM to analyze and summarize the conversation
        # with focus on graph-oriented database facts
        ...
        try:
            if database_summary:
                await self.graphiti_client.add_episode(
                    name=(f"Database_Facts_{user_id}_{database_name}_"
                          f"{datetime.now().strftime('%Y%m%d_%H%M%S')}"),
                    episode_body=f"Database: {database_name}\n{database_summary}",
                    source=EpisodeType.message,
                    reference_time=datetime.now(),
                    source_description=(f"Graph-oriented facts about "
                                         f"Database: {database_name} "
                                         f"from User: {user_id} interaction")
                )
