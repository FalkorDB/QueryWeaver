...
# =============================================================================
# These functions provide non-streaming alternatives for the SDK, returning
# structured results instead of async generators.

async def query_database_sync(  # pylint: disable=too-many-locals,too-many-statements,too-many-branches
    user_id: str,
    graph_id: str,
    ...
    overall_start = time.perf_counter()
    logging.info("SDK Query: %s", sanitize_query(queries_history[-1]))
    
    # Initialize memory tool if enabled
    memory_tool = None
    if chat_data.use_memory:
        memory_tool = await MemoryTool.create(user_id, graph_id)
    ...
    find_task = asyncio.create_task(find(graph_id, queries_history, db_description))
    relevancy_task = asyncio.create_task(agent_rel.get_answer(queries_history[-1], db_description))
    ...
    if answer_rel["status"] != "On-topic":
        ...
        return QueryResult(
            sql_query="",
            results=[],
            ai_response=f"Off topic question: {answer_rel['reason']}",
            confidence=0.0,
            is_valid=False,
            execution_time=time.perf_counter() - overall_start,
        )
    
    # Query is on-topic, get relevant tables
    result = await find_task
    ...
    if is_destructive and not general_graph:
        return QueryResult(
            sql_query=sql_query,
            results=[],
            ai_response=f"This {sql_type} operation requires confirmation before execution.",
            confidence=confidence,
            is_valid=True,
            is_destructive=True,
    ...
    if is_destructive and general_graph:
        return QueryResult(
            sql_query=sql_query,
            results=[],
            ai_response="Destructive operations are not allowed on demo databases.",
            confidence=confidence,
            is_valid=True,
            is_destructive=True,
            requires_confirmation=False,
...
async def refresh_database_schema_sync(user_id: str, graph_id: str):
    """
    ...
    """
    from queryweaver_sdk.models import RefreshResult
