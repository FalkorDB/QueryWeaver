...
This file demonstrates how to use the async version of the QueryWeaver Python library
for high-performance applications that can benefit from async/await patterns.
"""
...
async def concurrent_queries_example():
    ...
    try:
        # Load database first
        await client.load_database("analytics", "postgresql://user:pass@localhost/analytics")
        ...
        tasks = [
            client.text_to_sql("analytics", query) 
...
async def context_manager_example():
    ...
    async with AsyncQueryWeaverClient(
        ...
        load_tasks = [
            ...
            client.load_database("customers", "postgresql://user:pass@host/customers")
        ]
...
async def batch_processing_example():
    ...
    async with client:
        ...
        await client.load_database("reporting", "postgresql://user:pass@host/reporting")
        ...
        for i in range(0, len(query_batch), chunk_size):
            ...
            chunk_tasks = [
                client.query("reporting", query, execute_sql=False)
                for query in chunk
            ]
        ...
        print(f"Successfully processed {successful}/{len(query_batch)} queries")
...
async def streaming_example():
    ...
    try:
        await client.load_database("realtime", "postgresql://user:pass@host/realtime")
