...
Pylint: tests need to access protected members and define fixtures that are
intentionally re-used as parameters in test functions.
"""
...
from queryweaver import AsyncQueryWeaverClient, create_async_client  # pylint: disable=import-error
...
@pytest.fixture
def _async_client(_mock_falkordb):
...
class TestAsyncLoadDatabase:
    ...
    @pytest.mark.asyncio
    async def test_load_database_empty_name_raises_error(self, _async_client):
        """Test that empty database name raises ValueError."""
        with pytest.raises(ValueError, match="Database name cannot be empty"):
            await _async_client.load_database("", "postgresql://user:pass@host/db")

    @pytest.mark.asyncio
    async def test_load_database_empty_url_raises_error(self, _async_client):
        """Test that empty database URL raises ValueError."""
        with pytest.raises(ValueError, match="Database URL cannot be empty"):
            await _async_client.load_database("test", "")

    @pytest.mark.asyncio
    async def test_load_database_invalid_url_raises_error(self, _async_client):
        """Test that invalid database URL raises ValueError."""
        with pytest.raises(ValueError, match="Unsupported database URL format"):
            await _async_client.load_database("test", "invalid://url")

    @pytest.mark.asyncio
    @patch("queryweaver.AsyncQueryWeaverClient._load_database_async")
    async def test_load_database_success(self, mock_load_async, _async_client):
        """Test successful async database loading."""
        mock_load_async.return_value = True

        result = await _async_client.load_database(
            "test", "postgresql://user:pass@host/db"
        )
        assert result is True
        assert "test" in _async_client._loaded_databases

    @pytest.mark.asyncio
    @patch("queryweaver.AsyncQueryWeaverClient._load_database_async")
    async def test_load_database_failure(self, mock_load_async, _async_client):
        ...
        with pytest.raises(RuntimeError, match="Failed to load database schema"):
            await _async_client.load_database(
                "test", "postgresql://user:pass@host/db"
            )
...
class TestAsyncTextToSQL:
    ...
    @pytest.mark.asyncio
    async def test_text_to_sql_empty_query_raises_error(self, _async_client):
        """Test that empty query raises ValueError."""
        with pytest.raises(ValueError, match="Query cannot be empty"):
            await _async_client.text_to_sql("test", "")

    @pytest.mark.asyncio
    async def test_text_to_sql_database_not_loaded_raises_error(self, _async_client):
        """Test that unloaded database raises ValueError."""
        with pytest.raises(ValueError, match="Database 'test' not loaded"):
            await _async_client.text_to_sql("test", "Show me users")

    @pytest.mark.asyncio
    @patch("queryweaver.AsyncQueryWeaverClient._generate_sql_async")
    async def test_text_to_sql_success(self, mock_generate_async, _async_client):
        """Test successful async SQL generation."""
        # Add database to loaded set
        _async_client._loaded_databases.add("test")
        mock_generate_async.return_value = "SELECT * FROM users;"

        result = await _async_client.text_to_sql("test", "Show me all users")
        assert result == "SELECT * FROM users;"

    @pytest.mark.asyncio
    @patch("queryweaver.AsyncQueryWeaverClient._generate_sql_async")
    async def test_text_to_sql_with_instructions(self, mock_generate_async, _async_client):
        """Test async SQL generation with instructions."""
        _async_client._loaded_databases.add("test")
        mock_generate_async.return_value = "SELECT * FROM users LIMIT 10;"

        result = await _async_client.text_to_sql(
            "test",
            "Show me users",
...
class TestAsyncQuery:
    ...
    @pytest.mark.asyncio
    async def test_query_empty_query_raises_error(self, _async_client):
        """Test that empty query raises ValueError."""
        with pytest.raises(ValueError, match="Query cannot be empty"):
            await _async_client.query("test", "")

    @pytest.mark.asyncio
    async def test_query_database_not_loaded_raises_error(self, _async_client):
        """Test that unloaded database raises ValueError."""
        with pytest.raises(ValueError, match="Database 'test' not loaded"):
            await _async_client.query("test", "Show me users")

    @pytest.mark.asyncio
    @patch("queryweaver.AsyncQueryWeaverClient._query_async")
    async def test_query_success(self, mock_query_async, _async_client):
        ...
        result = await _async_client.query("test", "Show me all users")
        assert result["sql_query"] == "SELECT * FROM users;"
        assert len(result["results"]) == 1

    @pytest.mark.asyncio
    @patch("queryweaver.AsyncQueryWeaverClient._query_async")
    async def test_query_without_execution(self, mock_query_async, _async_client):
        ...
        result = await _async_client.query(
...
class TestAsyncUtilityMethods:
    ...
    def test_list_loaded_databases_empty(self, _async_client):
        """Test listing loaded databases when none are loaded."""
        result = _async_client.list_loaded_databases()
        assert result == []

    def test_list_loaded_databases_with_data(self, _async_client):
        """Test listing loaded databases with data."""
        _async_client._loaded_databases.add("db1")
        _async_client._loaded_databases.add("db2")
    ...
    @pytest.mark.asyncio
    async def test_get_database_schema_not_loaded_raises_error(self, _async_client):
        """Test that schema retrieval for unloaded database raises ValueError."""
        with pytest.raises(ValueError, match="Database 'test' not loaded"):
            await _async_client.get_database_schema("test")

    @pytest.mark.asyncio
    @patch("queryweaver.AsyncQueryWeaverClient._get_schema_async")
    async def test_get_database_schema_success(self, mock_schema_async, _async_client):
        """Test successful async schema retrieval."""
        _async_client._loaded_databases.add("test")
        expected_schema = {"tables": ["users", "orders"]}
        mock_schema_async.return_value = expected_schema

        result = await _async_client.get_database_schema("test")
        assert result == expected_schema

    @pytest.mark.asyncio
    async def test_close_method(self, _async_client):
        """Test async client close method."""
        # Should not raise any errors
        await _async_client.close()

    @pytest.mark.asyncio
    async def test_context_manager(self, _mock_falkordb):
...
class TestCreateAsyncClient:
    ...
    def test_create_async_client_success(self, _mock_falkordb):
    ...
    def test_create_async_client_with_additional_args(self, _mock_falkordb):
